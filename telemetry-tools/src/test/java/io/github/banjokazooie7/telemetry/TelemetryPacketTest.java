...
class TelemetryPacketTest {
    ...
    void constructsWithValidParams() {
        ...
        assertEquals(1, pkt.sequenceNumber());
    ...
    void rejectsNegativeSequenceNumber() {
        assertThrows(IllegalArgumentException.class, () ->
                new TelemetryPacket(-1, 0, "TGT", 0, 0, 0, 0, "UPDATE"));
    }

    @Test
    void rejectsNullEntityId() {
        assertThrows(IllegalArgumentException.class, () ->
                new TelemetryPacket(0, 0, null, 0, 0, 0, 0, "UPDATE"));
    }

    @Test
    void rejectsBlankEntityId() {
        assertThrows(IllegalArgumentException.class, () ->
                new TelemetryPacket(0, 0, "  ", 0, 0, 0, 0, "UPDATE"));
    }

    @Test
    void rejectsNullEventType() {
        assertThrows(IllegalArgumentException.class, () ->
                new TelemetryPacket(0, 0, "TGT", 0, 0, 0, 0, null));
    }

    @Test
    void rejectsBlankEventType() {
        assertThrows(IllegalArgumentException.class, () ->
                new TelemetryPacket(0, 0, "TGT", 0, 0, 0, 0, "  "));
    }

    @Test
    void recordEqualityWorks() {
        TelemetryPacket a = new TelemetryPacket(1, 1000, "TGT-1", 10, 20, 30, 40, "UPDATE");
        TelemetryPacket b = new TelemetryPacket(1, 1000, "TGT-1", 10, 20, 30, 40, "UPDATE");
        assertEquals(a, b);
    }
    ...
    void toStringContainsKeyFields() {
        TelemetryPacket pkt = new TelemetryPacket(
                42, 5000, "TGT-042", 1500.0, 3000.0, 250.0, 0.0, "RADAR_SWEEP");
    ...
    @Test
    void zeroSequenceNumberIsValid() {
        TelemetryPacket pkt = new TelemetryPacket(
                0, 0, "TGT-1", 0, 0, 0, 0, "START");
        assertEquals(0, pkt.sequenceNumber());
    }

    @Test
    void negativePositionIsValid() {
        TelemetryPacket pkt = new TelemetryPacket(
                5, 100, "TGT-1", -500.0, -300.0, -10.0, -20.0, "UPDATE");
        assertEquals(-500.0, pkt.xMeters(), EPSILON);
        assertEquals(-300.0, pkt.yMeters(), EPSILON);
    }
}
